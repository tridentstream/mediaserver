...
class Options(usage.Options):
    ...
    optParameters = [
        ["description", "e", "tcp:45477:interface=0.0.0.0", "Twisted server description"],
        ["reverseproxy", "r", "smart", "Set reverse proxy mode (uses x-forwarded headers), values are on, off and smart"],
    ]
...
class ServiceMaker(object):
    ...
    def makeService(self, options):
        ...
        if options['stdout']:
            ...
            handler = logging.handlers.RotatingFileHandler('tridentstream.log', maxBytes=50 * 1024 * 1024, backupCount=5)
        ...
        if options['reverseproxy'] == 'on':
            asgiresource = ASGIResource(application, use_proxy_headers=True, use_x_sendfile=True)
        elif options['reverseproxy'] == 'off':
            asgiresource = ASGIResource(application, use_proxy_proto_header=True, use_x_sendfile=True)
        else:
            asgiresource = ASGIResource(application, automatic_proxy_header_handling=True, use_x_sendfile=True)
        r = Root(asgiresource)
